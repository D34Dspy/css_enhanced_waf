...
SH_DECL_HOOK2_void(IServerGameDLL, GameFrame, SH_NOATTRIB, false, bool, bool);
...
# define SOURCEPAWN_DLL "sourcepawn.vm"
	...
	char myerror[255];
	g_SMAPI->PathFormat(file, sizeof(file), "./bin/sourcemodsourcepawnvm.%s", PLATFORM_LIB_EXT);
	...
	if (!g_JIT)
	{
		if (error && maxlength)
		{
			ke::SafeSprintf(error, maxlength, "%s (failed to load bin/sourcemodsourcepawnvm.%s)", myerror, PLATFORM_LIB_EXT);
		}

		ke::SafeSprintf(error, maxlength, "attempting fallback load ...", myerror, PLATFORM_LIB_EXT);

		g_SMAPI->PathFormat(file, sizeof(file), "./bin/libsourcemodsourcepawnvm.%s", PLATFORM_LIB_EXT);

		g_JIT = ke::SharedLib::Open(file, myerror, sizeof(myerror));
		if (!g_JIT)
		{
			if(error && maxlength)
				ke::SafeSprintf(error, maxlength, "%s (failed to load fallback bin/sourcemodsourcepawnvm.%s)", myerror, PLATFORM_LIB_EXT);
			return false;
		}
	}
...
bool broadcast_load(IExtension* pExt) {
	if (!pExt)
	{
		logger->LogError(" [1337] extension is NULL");
		return false;
	}
	if(!pExt->IsLoaded())
	{
		logger->LogError(" [1337] extension not loaded in the first place");
		return false;
	}
	char err[1024];
	if(!pExt->IsRunning(err, sizeof(err)))
	{
		logger->LogError(" [1337] extension not running due to error: %s", err);
		return false;
	}
	return true;
}
...
void SourceModBase::DoGlobalPluginLoads()
{
	...
	// {
		...
		// ke::SafeSprintf(path, sizeof(path), "./bin/%s.%s", smext[i], PLATFORM_LIB_EXT);
		// ke::StringPrintf(" [1337] attempting load %s\n", path);
		// if(extsys->LoadAutoExtension(path, false))
			// continue;
	// }
	...
	char extensionpath[1024];
	char fallbackpath[1024];
	int len = sizeof(smext) / sizeof(smext[0]);
	for(int i = 0; i < len; i++) {
		ke::SafeSprintf(extensionpath, sizeof(extensionpath), "./bin/lib%s.%s", smext[i], PLATFORM_LIB_EXT);
		ke::SafeSprintf(fallbackpath, sizeof(extensionpath), "./bin/%s.%s", smext[i], PLATFORM_LIB_EXT);
		logger->LogError(" [1337] attempting load %s (with fallback %s)\n", extensionpath, fallbackpath);
		if(broadcast_load(extsys->LoadAutoExtension(extensionpath, fallbackpath, false)))
			continue;
	}

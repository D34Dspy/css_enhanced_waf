...
bool CoreProviderImpl::LoadBridge(char *error, size_t maxlength)
{
	...
	char myerror[255];
	g_SMAPI->PathFormat(file, sizeof(file), "./bin/sourcemodlogic.%s", PLATFORM_LIB_EXT);
	logic_ = ke::SharedLib::Open(file, myerror, sizeof(myerror));
	if (!logic_) {
		ke::SafeSprintf(error, maxlength, "failed to load %s: %s", file, myerror);
		g_SMAPI->PathFormat(file, sizeof(file), "./bin/libsourcemodlogic.%s", PLATFORM_LIB_EXT);
		logic_ = ke::SharedLib::Open(file, myerror, sizeof(myerror));
		if(!logic_) {
			ke::SafeSprintf(error, maxlength, "failed to load fallback %s: %s", file, myerror);
			return false;
		}
	}

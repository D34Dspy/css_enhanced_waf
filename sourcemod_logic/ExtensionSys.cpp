...
EXPORT IExtensionSys* SourceMod_ExtensionManager() {
	return &g_Extensions;
}
			...
			if (!libsys->IsPathFile(path))
			{
				...
				if (!libsys->IsPathFile(path))
				{
					g_pSM->BuildPath(Path_SM,
						...
						"extensions/" PLATFORM_ARCH_FOLDER "%s." PLATFORM_LIB_EXT,
						filename);
					/* Special case for css enhanced built in sourcemod located inside bin directory of root */
					if (!libsys->IsPathFile(path))
					{
						g_pSM->Format(path, sizeof(path), "%s.%s", filename, PLATFORM_LIB_EXT);
						if (!libsys->IsPathFile(path))
						{
							g_pSM->Format(path, sizeof(path), "%s.%s", filename, PLATFORM_LIB_EXT);
							if (!libsys->IsPathFile(path))
							{
								ke::StringPrintf(" [1337] cannot find executable extension file %s (last attempt: %s\n)", filename, path);
							}
						}
					}
				}
...
IExtension *CExtensionManager::LoadAutoExtension(const char *name, const char* fallback, bool bErrorOnMissing) {
	logger->LogError("auto loading %s with fallback %s", name, fallback);
	if(libsys->PathExists(name)) 
	{
		return LoadAutoExtension(name, bErrorOnMissing);
	}
	else if(libsys->PathExists(fallback)) 
	{
		return LoadAutoExtension(fallback, bErrorOnMissing);
	}
	return nullptr;
}
...
{
	logger->LogError("auto loading %s", path);
	
	if (strcmp(path, "clientprefs.ext") == 0 || strcmp(path, "clientprefs.ext.so") == 0)
		return LoadAutoExtension("./bin/sourcemodclientprefs.so", "./bin/libsourcemodclientprefs.so");

	if (strcmp(path, "sdktools.ext") == 0 || strcmp(path, "sdktools.ext.so") == 0)
		return LoadAutoExtension("./bin/sourcemodsdktools.so", "./bin/libsourcemodsdktools.so");

	if (strcmp(path, "topmenus.ext") == 0 || strcmp(path, "topmenus.ext.so") == 0)
		return LoadAutoExtension("./bin/sourcemodtopmenus.so", "./bin/libsourcemodtopmenus.so");

	/* Remove platform extension if it's there. Compat hack. */
	const char *ext = libsys->GetFileExtension(path);
	if (ext && V_strcmp(ext, PLATFORM_LIB_EXT) == 0)
	{
...
IExtension *CExtensionManager::LoadExtension(const char *name, const char* fallback,
	char *error,
	size_t maxlength) {
	logger->LogError("loading %s with fallback %s", name ,fallback);
	if(libsys->PathExists(name)) 
	{
		return LoadExtension(name, error, maxlength);
	}
	else if(libsys->PathExists(fallback)) 
	{
		return LoadExtension(fallback, error, maxlength);
	}
	return nullptr;
}
...
{
	logger->LogError("loading %s", file);

	if (strcmp(file, "clientprefs.ext") == 0 || strcmp(file, "clientprefs.ext.so") == 0)
		return LoadExtension("./bin/sourcemodclientprefs.so", "./bin/libsourcemodclientprefs.so", error, maxlength);

	if (strcmp(file, "sdktools.ext") == 0 || strcmp(file, "sdktools.ext.so") == 0)
		return LoadExtension("./bin/sourcemodsdktools.so", "./bin/libsourcemodsdktools.so", error, maxlength);

	if (strcmp(file, "topmenus.ext") == 0 || strcmp(file, "topmenus.ext.so") == 0)
		return LoadExtension("./bin/sourcemodtopmenus.so", "./bin/libsourcemodtopmenus.so", error, maxlength);
...
void CExtensionManager::AddDependency(IExtension *pSource, const char *file, const char* fallback, bool required, bool autoload)
{
	/* This function doesn't really need to do anything now.  We make sure the 
	 * other extension is loaded, but handling of dependencies is really done 
	 * by the interface fetcher.
	...
	if (required || autoload)
	{
		LoadAutoExtension(file, fallback);
	}
}
											...
											size_t maxlength)
{
	...
	logger->LogError("%s %s", filename, filepath);

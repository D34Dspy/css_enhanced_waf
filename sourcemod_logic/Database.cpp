...
// using namespace std::chrono_literals; // c++11!
...
IDBDriver *DBManager::FindOrLoadDriver(const char *name)
{
	...
	g_pSM->Format(filename, sizeof(filename), "./bin/sourcemod%s", name);
	g_pSM->Format(fallback, sizeof(filename), "./bin/libsourcemod%s", name);

	IExtension *pExt = g_Extensions.LoadAutoExtension(filename, fallback);

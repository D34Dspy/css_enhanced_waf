...
public: //IExtensionManager
	IExtension *LoadExtension(const char *path, 
		char *error,
		size_t maxlength);
	IExtension *LoadExtension(const char *path, const char* fallback,
		char *error,
		size_t maxlength);
...
public:
	...
	IExtension *LoadAutoExtension(const char *name, const char* fallback, bool bErrorOnMissing=true);
	...
	void AddDependency(IExtension *pSource, const char *file, const char* fallback, bool required, bool autoload);
	void TryAutoload();

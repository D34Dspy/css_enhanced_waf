...
int portable_stricmp2(const char *s1, const char *s2) {
    while (*s1 && (tolower(static_cast<unsigned char>(*s1)) == 
...
mm_DetectGameInformation()
{
	...
	while (!feof(fp) && fgets(buffer, sizeof(buffer), fp) != NULL)
	{
		...
		if (portable_stricmp2(buffer, "SearchPaths") == 0)
			search = true;
		...
		if (portable_stricmp2(key, "Game") != 0 && portable_stricmp2(key, "GameBin") != 0)
			continue;
		...
		if (portable_stricmp2(key, "GameBin") == 0)

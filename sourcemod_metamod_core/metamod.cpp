...
static bool broadcast_preload(const char* filename) {
	bool already;
	char error[4096];
	auto id = g_PluginMngr.Load(filename, Pl_File, already, error, sizeof(error));
	if (id < Pl_MinId || g_PluginMngr.FindById(id)->m_Status < Pl_Paused)
	{
		mm_LogMessage("[1337] Failed to preload plugin %s.  %s", filename, error);
		return false;
	}
	return true;
};
...
DoInitialPluginLoads()
{
	...
	char cwd[1024];
	getcwd(cwd, 1024);
	if (!broadcast_preload("./bin/sourcemodcore.so"))
		if (!broadcast_preload("./bin/libsourcemodcore.so"))
			provider->DisplayError("source mod core could not be loaded!\n");
